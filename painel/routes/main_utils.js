...
<head>
  ...
  <style>
    .product-card-variations{display:flex;flex-direction:column;align-items:center;gap:6px;margin:7px 0 10px;}
    .product-card-variation-colors{display:flex;justify-content:center;gap:6px;flex-wrap:wrap;}
    .product-card-color-dot{width:16px;height:16px;border-radius:50%;border:1px solid #cbd5e1;box-shadow:0 0 0 1px #fff inset;}
    .product-card-variation-tags{display:flex;justify-content:center;gap:5px;flex-wrap:wrap;}
    .product-card-variation-tags span{border:1px solid #e5e7eb;background:#f9fafb;border-radius:999px;padding:3px 7px;font-size:11px;font-weight:700;color:#374151;}
  </style>
</head>
<body>
...
(function(){
  ...
  function shouldShowVariationsOnCard(p){
    ...
    return value === 'sim' || value === 'yes' || value === 'true' || value === '1' || value === 'on';
  }
  ...
  function productVariationsCardHtml(p){
    ...
    const colorItems = Array.isArray(v.colors)
      ? v.colors.map(c => ({ name: String(c?.name || '').trim(), hex: String(c?.hex || '').trim() })).filter(c => c.name)
      : lines(v.colorsText).map(line => {
          const parts = line.split('|').map(x => x.trim());
          return { name: parts[0] || '', hex: parts[1] || '' };
        }).filter(c => c.name);

    const combos = Array.isArray(v.combinations) ? v.combinations : [];
    ...
    const dots = colors.length
      ? '<div class="product-card-variation-colors">' + colors.map(c => {
          const hex = /^#?[0-9a-f]{3,8}$/i.test(c.hex || '') ? (c.hex.startsWith('#') ? c.hex : '#' + c.hex) : '';
...
(function(){
  ...
  function ensureMenu(){
    ...
    ['nav-desktop','nav-mobile'].forEach(function(id){
      ...
      // V8.2.2.6: padroniza a posição do Favoritos.
      // Mesmo que o menu da home carregue categorias depois, cada atualização move o link
      // para o final do menu, imediatamente antes da busca no desktop.
...
function catalogMenuRuntimeSource(){
...
function ensureSiteAnalyticsRuntime(siteDir){
  try {
    ...
    const tag = '\n<script src="/js/quality-analytics.js?v=20260830-3"></script>\n';
...
function ensureCatalogMenuRuntime(siteDir){
  try {
    const jsDir = path.join(siteDir, 'js');
    fs.mkdirSync(jsDir, { recursive:true });
    writeFileUtf8(path.join(jsDir, 'catalog-menu.js'), catalogMenuRuntimeSource());

    const targets = ['index.html', 'categoria.html'];
    for (const name of targets) {
      const file = path.join(siteDir, name);
      if (!fs.existsSync(file)) continue;
      let html = readFileUtf8(file);
      if (!html) continue;
      html = html.replace(/\s*<script[^>]+src=["']\/?js\/catalog-menu\.js[^"']*["'][^>]*><\/script>/gi, '');
      const tag = '\n<script src="/js/catalog-menu.js?v=20260823-1"></script>\n';
      if (/<\/body>/i.test(html)) html = html.replace(/<\/body>/i, tag + '</body>');
      else html += tag;
      writeFileUtf8(file, html);
    }
    ...
    console.warn(`⚠️ Falha ao instalar sincronizador do menu: ${e.message}`);
    return false;
  }
}
...
export function syncGeneratedHeaderIntoIndex(siteDir){
  try {
    ...
    let indexHtml = readFileUtf8(indexPath);
    ...
    if (oldHeader.length) {
      ...
      console.log('✅ Header estático da Home sincronizado.');
    } else {
      // A Home atual usa <div id="header"></div> + js/main.js. Nesse modo não
      // tentamos substituir um header inexistente; instalamos o reconciliador
      // externo que roda APÓS o carregamento assíncrono do cabeçalho.
      console.log('ℹ️ Home usa header dinâmico; sincronização ficará a cargo de /js/catalog-menu.js.');
    }
