(()=>{
  ...
  const today=()=>window.QualityDate?.today?.()||(()=>{const now=new Date(),year=now.getFullYear(),month=String(now.getMonth()+1).padStart(2,'0'),day=String(now.getDate()).padStart(2,'0');return `${year}-${month}-${day}`})();
  ...
  function exportReportHtml(logoSrc=''){
  ...
  async function exportImage(){
    const logoSrc=await window.QualityExportBrand?.getLogoDataUrl?.()||'',width=1600,rowHeight=44,height=Math.max(700,260+exportRows().length*rowHeight),html=`<div xmlns="http://www.w3.org/1999/xhtml"><style>${exportStyles()}body{width:${width}px}.report{width:${width}px}</style>${exportReportHtml(logoSrc)}</div>`,svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><foreignObject width="100%" height="100%">${html}</foreignObject></svg>`,blob=new Blob([svg],{type:'image/svg+xml;charset=utf-8'}),url=URL.createObjectURL(blob),img=new Image();
    img.onload=()=>{const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);ctx.drawImage(img,0,0);URL.revokeObjectURL(url);canvas.toBlob(file=>{if(!file){alert('Não foi possível gerar a imagem.');return}const imageUrl=URL.createObjectURL(file),a=document.createElement('a');a.href=imageUrl;a.download=`contas-a-pagar-${today()}.png`;a.style.display='none';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(imageUrl),1500)},'image/png')};img.onerror=()=>{URL.revokeObjectURL(url);alert('Não foi possível gerar a imagem deste relatório. Use a exportação em PDF.')};img.src=url;closeExportMenu();
  }
  ...
  document.querySelectorAll('[data-summary-filter]').forEach(card=>card.onclick=()=>{const type=card.dataset.summaryFilter;$('clearPayableFilters').click();if(type==='total-period'){setPayableView('all',{reload:false});duePreset='month';}if(type==='open'){setPayableView('active',{reload:false});$('payableStatus').value='open';}if(type==='today'){setPayableView('active',{reload:false});duePreset='today';}if(type==='overdue'){setPayableView('active',{reload:false});duePreset='overdue';}if(type==='paid-month'){setPayableView('paid',{reload:false});$('payableStatus').value='paid';duePreset='month';}if(['this-week','next-week','next-30'].includes(type)){const start=new Date(`${today()}T12:00:00`);let from=new Date(start),to=new Date(start);if(type==='this-week')to.setDate(to.getDate()+7);if(type==='next-week'){from.setDate(from.getDate()+8);to=new Date(start);to.setDate(to.getDate()+14)}if(type==='next-30'){to.setDate(to.getDate()+30)}const iso=value=>window.QualityDate?.dateKey?.(value)||`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,'0')}-${String(value.getDate()).padStart(2,'0')}`;$('payableDueFrom').value=iso(from);$('payableDueTo').value=iso(to);duePreset='';$('payableAdvancedFilters').hidden=false;}document.querySelectorAll('[data-due-preset]').forEach(x=>x.classList.toggle('active',x.dataset.duePreset===duePreset));load().catch(e=>alert(e.message))});
