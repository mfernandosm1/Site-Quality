(() => {
  ...
  const labels = { open:'Em aberto', partial:'Parcial', paid:'Recebido', cancelled:'Cancelado', reversed:'Estornado', created:'Título criado', received:'Recebimento registrado' };
  ...
  function filteredTitles() {
    const query = document.getElementById('receivableSearch').value.trim().toLowerCase();
    ...
    return titles.filter(title => {
      ...
      if (query && ![title.number,title.customerName,title.originLabel].some(value => String(value || '').toLowerCase().includes(query))) return false;
  ...
  function render() {
    ...
    body.innerHTML = rows.map(title => {
      ...
      return `<tr class="receivable-row" tabindex="0" data-open="${esc(title.id)}" aria-label="Abrir título ${esc(title.number)}"><td><b>${esc(title.number)}</b></td><td>${esc(title.customerName)}<small>${esc(documentMask(title.customerDocument))}</small></td><td>${esc(title.originLabel || '—')}</td><td>${next ? date(next.dueDate) : '—'}</td><td>${money(title.netValue)}</td><td>${money(title.openBalance)}</td><td><span class="finance-status ${title.status==='paid'?'':'off'}">${esc(labels[title.status] || title.status)}</span></td></tr>`;
  ...
  function customerBlock(customer,title,creditSummary={}) {
    const c=customer || {};
    const content=`<div class="receivable-detail-grid"><div><span>Nome</span><b>${esc(c.name || title.customerName)}</b></div><div><span>CPF/CNPJ</span><b>${esc(documentMask(c.document || title.customerDocument))}</b></div><div><span>Telefone</span><b>${esc(c.mobile || c.phone || '—')}</b></div><div><span>E-mail</span><b>${esc(c.email || '—')}</b></div><div><span>Saldo em aberto</span><b>${money(creditSummary.openBalance)}</b></div><div><span>Saldo vencido</span><b>${money(creditSummary.overdueBalance)}</b></div></div>`;
    return fold('ficha-cliente','Cliente',content,true);
  }
  ...
  function operationBlock(operation,title) {
    if (!operation) return fold('ficha-operacao','Operação',`<p>${esc(title.originLabel || 'Operação não localizada.')}</p>`,true);
    ...
    return `${fold('ficha-operacao','Venda / Operação',operationInfo,true)}${fold('ficha-produtos','Produtos e serviços',`<div class="receivable-product-table">${products}</div>`,true)}${fold('ficha-pagamentos','Pagamentos originais',paymentRows,true)}${operation.type==='service_order'?serviceOrderBlock(serviceOrder):''}`;
  }
  ...
  function serviceOrderBlock(os={}) {
    ...
    return fold('ficha-os','Ordem de Serviço',content,true);
  }
  ...
  function receiptMethods(receipt={}) {
    ...
    return payments.map(item=>`${catalogName('paymentMethods',item.paymentMethodId)} · ${money(item.value)}`).join(' | ');
  }
  ...
  function timelineBlock(data={}) {
    ...
    const receiptEvents=(data.receipts||[]).map(item=>({at:item.receivedDateTime||item.createdAt,label:`Recebimento ${item.receiptNumber||''} · ${money(item.value)}`,actor:item.createdBy,type:'received'}));
    ...
    return fold('ficha-timeline','Timeline completa',content,true);
  }

  function showDetail(data) {
    currentDetail=data;
    const title=data.item;
    detail.innerHTML = `<div class="receivable-drawer-header"><div><small>TÍTULO ${esc(title.number)}</small><h2>${esc(title.customerName)}</h2><p>${esc(title.originLabel || 'Origem operacional')}</p></div><button type="button" data-close aria-label="Fechar">×</button></div><nav class="receivable-drawer-nav"><a href="#ficha-operacao">Operação</a><a href="#ficha-cliente">Cliente</a><a href="#ficha-produtos">Produtos</a><a href="#ficha-parcelas">Parcelas</a><a href="#ficha-recebimentos">Recebimentos</a><a href="#ficha-timeline">Timeline</a></nav><div class="receivable-drawer-body"><section class="receivable-detail-summary"><div><span>Valor</span><b>${money(title.netValue)}</b></div><div><span>Saldo</span><b>${money(title.openBalance)}</b></div><div><span>Situação</span><b>${esc(labels[title.status] || title.status)}</b></div></section>${operationBlock(data.operation,title)}${customerBlock(data.customer,title,data.creditSummary)}${installmentsBlock(title)}${receiptsBlock(data.receipts||[])}${creditBlock(data.creditSummary)}${timelineBlock(data)}</div>`;
  ...
  function openReceiveModal(installmentId) {
    ...
    document.getElementById('receiveTitleLabel').textContent=`Título ${title.number} · ${title.customerName}`;
