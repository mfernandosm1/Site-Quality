(() => {
  ...
  const addDays = days => window.QualityDate?.addDays?.(today(),days) || (()=>{const d=new Date(`${today()}T12:00:00`);d.setDate(d.getDate()+days);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`})();
  ...
  function operationBlock(operation,title) {
    ...
    const products = items.map(item=>{const itemSeller=item.sellerNameSnapshot||item.sellerName||(operation.type==='sale'?(operation.sellerNameSnapshot||operation.snapshot?.seller?.name):'')||'—';return `<div class="receivable-product-row"><div><span>Produto / serviço</span><b>${esc(item.name || item.productName || 'Item')}</b></div><div><span>Qtd.</span><b>${Number(item.quantity||1)}</b></div><div><span>Unitário</span><b>${money(item.unitPrice||item.price)}</b></div><div><span>Desconto</span><b>${money(item.discount)}</b></div><div><span>Subtotal</span><strong>${money(itemTotal(item))}</strong></div>${item.notes||item.observation?`<div class="receivable-product-note">Observação: ${esc(item.notes||item.observation)}</div>`:''}<div class="receivable-product-note">Vendedor do item: ${esc(itemSeller)}</div></div>`}).join('') || '<p>Nenhum item localizado.</p>';
    const responsibleLabel=operation.type==='service_order'?'Técnico':'Vendedor principal';
    const responsibleName=operation.type==='service_order'?(serviceOrder.technicianName||'—'):(operation.sellerNameSnapshot||operation.snapshot?.seller?.name||'—');
    const operationInfo=`<div class="receivable-operation-head"><div><h3>${operation.type==='service_order'?'O.S.':'Venda'} nº ${esc(operation.number)}</h3><small>${esc(title.originLabel||'Operação comercial')}</small></div><span class="receivable-status-pill">${esc(operationStatus(operation))}</span></div><div class="receivable-detail-grid"><div><span>Abertura</span><b>${dateTime(operation.dates?.openedAt||operation.createdAt)}</b></div><div><span>Finalização</span><b>${dateTime(operation.dates?.finalizedAt||operation.finalizedAt)}</b></div><div><span>Operador</span><b>${esc(operation.finalizedBy||operation.updatedBy||operation.createdBy||'—')}</b></div><div><span>${responsibleLabel}</span><b>${esc(responsibleName)}</b></div><div><span>Total</span><b>${money(operation.total||operation.snapshot?.totals?.total)}</b></div><div><span>Situação financeira</span><b>${esc(operation.financialStatus||'—')}</b></div></div>${operation.notes?`<div class="receivable-notes-box"><b>Observação da operação</b><br>${esc(operation.notes)}</div>`:''}`;
