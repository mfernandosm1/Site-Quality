...
export default class ReceivablesService {
  ...
  list({ status='', customerId='', search='', paymentMethodId='', costCenterId='', accountId='', dueFrom='', dueTo='', duePreset='' } = {}) {
    ...
    return titles.map(item => {
      ...
      if (!QualitySearch.matches([item.number,item.customerName,item.originLabel].join(' '),term)) return false;
      return true;
  ...
  reconcileOperations(operations=[], actor='sistema') {
    ...
    for (const operation of Array.isArray(operations)?operations:[]) {
      ...
      const linkedTitles=this.list().filter(title=>operationReceivableIds.has(String(title.id))||String(title.originId||'')===String(operation.id));
      for (const title of linkedTitles) {
        if (['cancelled','reversed'].includes(title.status)) continue;
  ...
  reverseReceipt(id, receiptId, reason='', actor='painel') {
    ...
    const openCash=this.commerceService.findOpenSessionForUser(actorName,receipt.cashboxId);
    if(!openCash?.session) throw new Error(`Abra o caixa ${receipt.cashboxName||receipt.cashboxId||''} antes de estornar o recebimento.`);
    const installmentsDb=this.read('receivable-installments.json');
    ...
    for(const [index,payment] of (receipt.payments||[]).entries()) {
      ...
      const movement=this.commerceService.addCashMovement({
        ...
        description:`Estorno recebimento ${receipt.receiptNumber} · Título ${title.number} · ${why}`,
        ...
        metadata:{receiptId:receipt.id,receivableId:id,originalReceiptId:receipt.id,reason:why,operationId:title.originId||'',customerId:title.customerId||'',customerName:title.customerName||'',cashboxId:openCash.cashbox.id}
      });
    ...
    title.status=remainingReceipts.length?'partial':'open'; title.updatedAt=now(); title.updatedBy=actorName;
    this.write('receivables.json',titleDb);
