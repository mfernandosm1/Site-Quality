(()=>{
  ...
  const exportImage=()=>{const width=1500;const inner=`<div xmlns="http://www.w3.org/1999/xhtml"><style>${styles()}</style>${reportHtml()}</div>`;const probe=document.createElement('div');probe.style.cssText=`position:fixed;left:-10000px;top:0;width:${width}px;background:#fff;`;probe.innerHTML=`<style>${styles()}</style>${reportHtml()}`;document.body.appendChild(probe);const height=Math.max(850,Math.ceil(probe.scrollHeight+10));probe.remove();if(height>18000&&!confirm('O relatório ficou muito longo e a imagem será grande. Deseja continuar?'))return;const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><foreignObject width="100%" height="100%">${inner}</foreignObject></svg>`;const blob=new Blob([svg],{type:'image/svg+xml;charset=utf-8'});const url=URL.createObjectURL(blob);const img=new Image();img.onload=()=>{const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);ctx.drawImage(img,0,0);URL.revokeObjectURL(url);canvas.toBlob(file=>{if(!file){alert('Não foi possível gerar a imagem.');return}const a=document.createElement('a');a.href=URL.createObjectURL(file);a.download=`dre-${iso(new Date())}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1500)},'image/png')};img.onerror=()=>{URL.revokeObjectURL(url);alert('Não foi possível gerar a imagem. Use a exportação em PDF.')};img.src=url;closeMenu();};
