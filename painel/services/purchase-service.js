...
export default class PurchaseService {
  constructor({dataDir, productsFile, supplierService, inventoryService, payablesService}) {
    ...
    this.supplierService=supplierService; this.inventoryService=inventoryService; this.payablesService=payablesService;
  ...
  productCatalog(){
    ...
    if(this._productCatalogCache?.key===cacheKey)return this._productCatalogCache.rows;
    ...
    for(const p of (this.products().items||[])){
      ...
      const common={parentProductId:parentId,code:clean(p.code||p.codigo),sku:clean(p.sku||p.code||p.codigo),barcode:clean(p.barcode||p.gtin||p.ean||p.codigoBarras),reference:clean(p.reference||p.referencia),brand:clean(p.brand||p.marca||p.manufacturer),model:clean(p.model||p.modelo),stockControlled:controlled,allowNegative:p?.inventory?.allowNegative===true,currentCost,salePrice,marginPercent:currentCost>0&&salePrice>0?money(((salePrice-currentCost)/currentCost)*100):null,active:(p?.erp?.active!==undefined?p.erp.active:p.active)!==false,siteEnabled:(p?.virtualStore?.active!==undefined?p.virtualStore.active:p.active)!==false,hasVariationOptions:!isService&&this.hasVariationOptions(p),variationConfigurationPending:!isService&&this.hasVariationOptions(p)&&!variations.length};
  ...
  searchProducts(query=''){return this.productCatalog().filter(p=>p.active&&searchMatches([p.name,p.code,p.sku,p.barcode,p.reference,p.brand,p.model].join(' '),query)).slice(0,120);}
  ...
  complete(id,actor='painel'){const db=this.read('purchases.json');const item=db.items.find(x=>x.id===id);if(!item)throw new Error('Compra não encontrada.');if(item.status==='completed')return this.get(id);if(item.status==='reversed')throw new Error('Compra estornada não pode ser concluída.');if(item.completionKey)return this.get(id);item.completionKey=`PUR-COMPLETE-${id}`;item.updatedAt=now();this.write('purchases.json',db);
    ...
    try{
      ...
      const productsDb=this.products();for(const row of item.items){const p=(productsDb.items||[]).find(x=>String(x.id)===row.productId);if(!p)continue;p.commercial=p.commercial||{};p.commercial.costPrice=row.unitCost;const combos=Array.isArray(p?.variations?.combinations)?p.variations.combinations:[];const combo=row.variationLabel&&combos.length?combos.find(c=>String(c?.inventoryItemId||'')===String(row.inventoryItemId)):null;if(combo){combo.erpPrice=money(row.salePrice);}else{p.commercial.erpPrice=money(row.salePrice);p.commercial.salePrice=money(row.salePrice);p.commercial.cashPrice=money(row.salePrice);p.commercial.installmentPrice=money(row.salePrice);}if(row.stockControlled&&row.inventoryItemId){if(combo){const bal=this.inventoryService.consultarSaldo({inventoryItemId:row.inventoryItemId});combo.stock=bal.physicalQuantity;p.stock=combos.reduce((sum,c)=>sum+(Number(c?.stock)||0),0);}else{const bal=this.inventoryService.consultarSaldo({inventoryItemId:row.inventoryItemId});p.stock=bal.physicalQuantity;}}}this.writeFile(this.productsFile,productsDb);
