...
**Versão de referência do módulo:** `0.10.9-aniversariantes-reenvio-manual`  
**Data da documentação:** 22/08/2026  
**Status:** funcional em ambiente local, com envio automático, simulação, teste real e reenvio manual de falhas.
...
A execução automática continua tentando clientes com erro até o limite de **3 tentativas no mesmo dia**. Se o telefone for corrigido antes desse limite, uma tentativa automática posterior poderá concluir o envio normalmente.
...
### Reenvio manual após correção

Clientes com status de erro exibem o botão **Reenviar agora** na lista de aniversariantes. O fluxo recomendado é:

1. corrigir o telefone/celular no cadastro do cliente;
2. atualizar a tela de Aniversariantes;
3. clicar em **Reenviar agora** somente naquele cliente.

O reenvio manual ignora o limite de 3 tentativas automáticas, mas mantém as proteções contra duplicidade: um cliente ou número que já conste como enviado no dia não é disparado novamente. O resultado da tentativa manual também é gravado no histórico e nos logs.
...
- [x] Envio automático
- [x] Execução manual
- [x] Reenvio manual de cliente com erro após correção do número
