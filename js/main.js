function qualityIsStaticCategoryPage(){
  var p=(window.location.pathname||'/').replace(/\/+$/,'')||'/';
  return !!window.__QUALITY_STATIC_CATEGORY_PAGE__ || ['/smartphones','/acessorios','/eletronicos','/assistencia-tecnica'].indexOf(p)!==-1;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (headerContainer) {
    ...
    // Categorias físicas usam category-ui.js; main.js não deve tocar no header/nav.
    if (!qualityIsStaticCategoryPage()) initHeaderEvents();
  }
...
function carregarCategoriasNoHeader() {
  if (qualityIsStaticCategoryPage()) return;
...
(function loadQualityStorefrontExperienceV1(){
  ...
  script.src = '/js/storefront-enhancements.js?v=20260925-commerce1';
  script.defer = true;
